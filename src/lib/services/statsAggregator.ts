...
export async function generateDailyStats(date: string) {
    ...
    for (const row of res.rows) {
        ...
        const statsRes = await db.query(`
            ...
            FROM listings_normalized
            WHERE card_id = $1 AND language = $2 AND condition = $3 AND DATE(collected_at AT TIME ZONE 'America/Sao_Paulo') = $4
        `, [card_id, language, condition, date]);

        if (statsRes.rows.length > 0) {
            const stats = statsRes.rows[0];
            
            await db.query(`
                INSERT INTO daily_stats (date, card_id, language, condition, avg_price, min_price, max_price, listing_count)
                VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
                ON CONFLICT (date, card_id, language, condition) DO UPDATE SET
